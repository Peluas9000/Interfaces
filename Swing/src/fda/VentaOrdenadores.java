...
public class VentaOrdenadores extends JFrame implements ActionListener{
	...
	public VentaOrdenadores() {
		...
		//clickados por defecto 
		botonProcesador1.setSelected(true);
		botonMemoria2.setSelected(true);
		botonDisco3.setSelected(true);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()== añadir) {
			listaClientes.add(cliente.getText(), memoria);
		}
		if(e.getSource()== eliminar) {
			String nombreAEliminar = cliente.getText().trim();

			DefaultListModel<String> modelo = (DefaultListModel<String>) listaClientes.getModel();

			boolean encontrado = false;
			for(int i=0;i<modelo.size();i++) {
				String nombreLista=modelo.getElementAt(i);
				if(nombreAEliminar.equalsIgnoreCase(nombreLista)) {
					modelo.remove(i);
					encontrado=true;
					break;
				}
			...
			if(!encontrado) {
				  JOptionPane.showMessageDialog(this, 
					        "No se encontró el cliente: " + nombreAEliminar,
